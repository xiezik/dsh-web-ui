...
import { framePosition, rowOfTrack, rowOfTrackFor, trimTrack } from './spritesheet.ts'
...
export function PetSprite(props: PetSpriteProps): ReactPortal {
  ...
  // Fork: content-top anchor ratios per row (bubble/panel anchor) ride the
  // registry definition extension; done/failed bubbles auto-hide after a beat.
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // Fork: content-top anchor ratios come from the registry definition.
      setLayoutByRow(definition.layoutByRow ?? null)
    }
  ...
  useEffect(() => {
    ...
    const row = rowOfTrackFor(animation, rows.length)
    const def = tracks[animation] ?? tracks.running
    const track = trimTrack(def, rows[row] ?? def.frames.length)
    ...
    const tick = (ts: number): void => {
      ...
      // would render as a vanishing pet). Fork: tool tracks fall back to the
      // generic running rhythm when the pet atlas has no dedicated row.
      const row = rowOfTrackFor(animation, rows.length)
      const def = tracks[animation] ?? tracks.running
      const track = trimTrack(def, rows[row] ?? def.frames.length)
  ...
  // Fork: done/failed status bubbles auto-hide after 2.5s; active bubbles
  // (thinking/tool) stay visible while work progresses.
  useEffect(() => {
  ...
  const statusBubble = feedback === null && !hovered && sessionBubbles.length === 0
    ? snapshot?.bubble
    : undefined
  // Fork: content-aware overlay anchor — bubbles sit on the CONTENT top, not
  // the cell top (bust rows start low inside the cell, so anchoring to the
  // cell top would leave a large empty gap between the pet and its bubble).
  const activeRow = rowOfTrackFor(animation, rows.length)
  ...
  const float = (
    ...
    >
      ...
      {statusBubble !== undefined && !transientHidden && (
        <div
          key={'tool-' + (snapshot?.phase ?? '') + '-' + statusBubble}
          className={styles.bubble + ' ' + (styles[statusBubbleClass(snapshot?.phase ?? 'tool')] ?? styles.bubbleTool)}
          style={{ bottom: 'calc(100% - ' + contentTopPx + 'px + 6px)' }}
        >
          <span className={styles.bubbleMain}>{statusBubble}</span>
          <span className={styles.bubbleSub}>{props.t(subLabel(snapshot?.phase ?? 'tool'))}</span>
        </div>
      )}
      {/* Fork: 挂靠表现——底部地面投影（半身素材的“地面依托”） */}
