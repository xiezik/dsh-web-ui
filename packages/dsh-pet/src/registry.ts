...
/** Fork: tool-bust animations (rows 9-13 when the atlas declares them). */
export const TOOL_ANIMATIONS: readonly PetAnimation[] = ['fetching', 'searching', 'analyzing', 'building', 'chatting']
...
export const DEFAULT_TRACK_PATTERNS: Record<PetAnimation, {
  ...
  // Fork: tool-bust animations default to the generic running rhythm; pets
  // with dedicated rows override durations in their manifest tracks.
  fetching: { durations: [120, 120, 120, 120, 120, 220], loop: true },
  searching: { durations: [120, 120, 120, 120, 120, 220], loop: true },
  analyzing: { durations: [120, 120, 120, 120, 120, 220], loop: true },
  building: { durations: [120, 120, 120, 120, 120, 220], loop: true },
  chatting: { durations: [120, 120, 120, 120, 120, 220], loop: true },
}
...
export interface PetDefinition {
  ...
  manifestUrl: string
  /**
   * Fork: content-top anchor ratio per row (0..1 of the cell height), used to
   * anchor status bubbles/panels to the row's actual content top instead of
   * the cell top. Absent on pets whose content fills the cell.
   */
  layoutByRow?: number[]
}
...
export function resolvePetManifest(
  ...
  // Fork: keep extra tool-bust rows declared in `frames` (beyond the 9-row
  // contract) so pets with dedicated tool rows animate their full cells.
  const declaredFrames = Array.isArray(source.frames)
    ? source.frames.map((value, index) => finiteInt(value, DEFAULT_FRAME_COUNTS[index] ?? 8, columns))
    : []
  const rows = declaredFrames.length > PET_ROW_ORDER.length
    ? declaredFrames
    : DEFAULT_FRAME_COUNTS.map((fallback, index) => {
  ...
  // Fork: content-top anchor ratios (bubble/panel anchor); only whole arrays
  // of finite non-negative numbers are accepted.
  const rawLayout = source.layoutByRow
  const layoutByRow = Array.isArray(rawLayout) && rawLayout.length > 0
    && rawLayout.every((n) => typeof n === 'number' && Number.isFinite(n) && n >= 0)
    ? (rawLayout as number[])
    : undefined
  ...
  // Fork: resolve tool-bust tracks (defaults to the running rhythm; the
  // manifest may override durations via the same track syntax).
  for (const [toolIndex, animation] of TOOL_ANIMATIONS.entries()) {
    const row = PET_ROW_ORDER.length + toolIndex
    const pattern = DEFAULT_TRACK_PATTERNS[animation]
    const override = trackOverrides[animation]
    const durations = Array.isArray(override?.durations) && override.durations.length > 0
      ? override.durations.filter((value): value is number => typeof value === 'number' && Number.isFinite(value) && value > 0)
      : pattern.durations
    const frameCount = Math.max(1, Math.min(rows[row] ?? columns, columns))
    const sized = durations.length >= frameCount
      ? durations.slice(0, frameCount)
      : Array.from({ length: frameCount }, (_, index) => durations[index % durations.length]!)
    tracks[animation] = {
  ...
  return {
    ...
    ...(layoutByRow === undefined ? {} : { layoutByRow }),
    dir,
